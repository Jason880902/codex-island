...
CodexIsland 是一个原生 macOS 悬浮层，把 MacBook 刘海变成类似 Dynamic Island 的实时用量状态。支持 Kimi、Codex、Claude、Grok、GLM 五家订阅：悬停预览 5 小时窗口，点击展开完整面板，展示你在设置里选定的左右两个槽位服务的 5 小时与周窗口用量、重置时间、图表样式，以及从本地会话日志估算的美元成本和 token 吞吐量。
