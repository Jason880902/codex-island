...
/// is inferred from the message's `providerID` field: "openai" → .codex,
/// "anthropic" → .claude, the Zhipu family ("zhipu"/"z-ai"/"zai"/"bigmodel")
/// → .glm, and the xAI family ("xai"/"grok") → .grok; every other providerID
/// is ignored. IDs arrive lowercased from OpenCode, so matching is exact —
/// no case normalization here.
enum OpenCodeLogReader {
    ...
    private struct ParsedEvent {
        ...
        func tokenEvent() -> TokenEvent? {
            ...
            // Zhipu has shipped several provider ids across OpenCode
            // releases and user configs; all route to GLM.
            case "zhipu", "z-ai", "zai", "bigmodel": mapped = .glm
            case "xai", "grok": mapped = .grok
            default: return nil
            }
            return TokenEvent(
                provider: mapped,
