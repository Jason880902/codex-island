...
enum CostSummary {
    static func summarize(events: [TokenEvent], now: Date = Date()) -> ProviderCost {
        ...
        // 5h matches Kimi's rate-limit window; 7d matches both
        ...
        for event in events {
            ...
            // (ccusage parity); `billable` is input + output only, excluding
            // cache tokens. Persisting both lets the Settings toggle flip
            // the displayed figure instantly without re-scanning session logs.
    ...
    private static func prettyModelName(_ canonical: String) -> String {
        // Kimi: "kimi-code/kimi-for-coding" → "Kimi For Coding"
        if canonical.hasPrefix("kimi-code/") {
            let trimmed = String(canonical.dropFirst("kimi-code/".count))
            return trimmed.replacingOccurrences(of: "-", with: " ").capitalized
        }
