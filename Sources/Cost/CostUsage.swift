...
struct CostWindow {
    ...
    let tokens: Int
    /// Input + output only, excluding cache reads. Surfaced when
    /// `TokenCountMode.billable` is selected.
...
/// derived from local session logs, not the usage endpoint (neither
/// provider exposes this slice). Currently used by the prototype Repurpose
