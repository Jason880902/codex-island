...
final class AlertEngine: ObservableObject {
    ...
    enum Provider: String, Hashable, CaseIterable {
        ...
        /// UI-facing brand name. Used wherever a label switch used to
        /// hardcode "Kimi"/"Codex" — extend here when a provider is added.
    ...
    /// Per-provider severity for the peek pill content swap. Read by the
    /// pill; missing entries read as `.none`.
    @Published private(set) var severities: [Provider: Severity] = [:]
    ...
    func start() {
        ...
        let triggers: [AnyPublisher<Void, Never>] = [
            UsageStore.shared.$usage.map { _ in () }.eraseToAnyPublisher(),
    ...
    private func recompute() {
        ...
        // One input per known provider; only slot-assigned providers are
        // visible, so alerts/pulses stay scoped to what the user actually
        // monitors in the two island slots.
        let inputs: [AlertDecision.WindowInput] = Provider.allCases.map { provider in
            AlertDecision.WindowInput(
                provider: provider,
                visible: visibility.effectiveVisible(provider: provider),
                window: usage.usage[provider]?.fiveHour ?? .unknown
            )
        }
        ...
        if perWindowSeverity != self.severities { self.severities = perWindowSeverity }
