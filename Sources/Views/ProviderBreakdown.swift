...
// MARK: - Provider data access
//
// No per-provider switches live here anymore: brand color comes from
// `Provider.brandColor`, the display label from `Provider.displayName`,
// and the cost rows from `CostStore.cost(for:)` — adding a provider to
// the enum is all that's needed for this view to support it.
...
struct PerModelBreakdown: View {
    ...
    private var color: Color { provider.brandColor }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if rows.isEmpty {
                Spacer(minLength: 0)
                Text(L10n.tr("no %@ activity in last 5h or this week", provider.displayName))
