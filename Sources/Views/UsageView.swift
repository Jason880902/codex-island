...
///
/// Branches on the two slots from `ProviderVisibilityStore`:
///   - both set: two `ChartsBlock`s with a hairline divider (default).
///   - one set:  the live block on its slot side, hairline, then a
...
struct UsageView: View {
    ...
    var body: some View {
        let left = slots.leftProvider
        let right = slots.rightProvider

        HStack(spacing: 0) {
            switch (left, right) {
            case let (l?, r?):
                chartsBlock(l, seed: 1)
                hairline
                chartsBlock(r, seed: 3)
            case let (p?, nil):
                chartsBlock(p, seed: 1)
                hairline
                PerModelBreakdown(provider: p, metric: .tokens)
    ...
    /// Live charts for one slotted provider. Seeds stay slot-keyed (left 1,
    /// right 3) so the default Kimi-left/Codex-right layout renders the
    /// exact same spark fallback shapes as the pre-slot code did.
    private func chartsBlock(_ provider: AlertEngine.Provider, seed: Int) -> ChartsBlock {
        ChartsBlock(color: provider.brandColor,
                    usage: store.usage[provider] ?? .empty,
                    style: style, seed: seed, provider: provider)
    }
...
struct ChartsBlock: View {
    ...
    /// reauth-actionable sentinel — an expired token or a revoked session.
    /// Either tile alone could be a transient per-window failure, but a
    /// matching pair = the underlying token is genuinely unusable. Only
    /// kimi and claude carry auth sentinels; the other providers surface
    /// fetch errors inline on the tiles instead.
    private var needsReauth: Bool {
        switch provider {
        case .kimi:   return KimiCredentials.isTerminalAuthFailure(usage)
        case .claude: return ClaudeCredentials.isTerminalAuthFailure(usage)
    ...
    var body: some View {
        Group {
            if needsReauth {
                ...
                ReauthState(color: color, usage: usage, provider: provider)
            } else {
                HStack(spacing: 18) {
...
/// Shown in place of the sparkline tiles when a provider's token can no
/// longer be used — expired locally or rejected by the server. Both windows
/// carry a reauth-actionable sentinel; the dead numbers would only mislead,
/// so this centered prompt takes their place. Kimi offers one-click re-auth
/// when a `kimi` binary is discoverable (otherwise the exact manual command
/// from the sentinel); Claude has no in-app reauth, so it always takes the
/// static caption branch (`claude /login` is a terminal action).
struct ReauthState: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if provider == .kimi && KimiCredentials.canPromptReauth() {
                Text(L10n.tr("Kimi session expired"))
                    .font(Typography.label)
                    .foregroundStyle(.white.opacity(0.55))
                ReauthButton()
            } else {
                Text(usage.fiveHour.error ?? tokenExpiredMessage)
                    .font(Typography.label)
                    .foregroundStyle(.white.opacity(0.55))
    ...
    /// Fallback for the theoretical case where the 5h window lost its
    /// sentinel — `needsReauth` already guarantees an error string, so in
    /// practice this only documents the recovery command per provider.
    private var tokenExpiredMessage: String {
        provider == .claude
            ? ClaudeCredentials.tokenExpiredMessage
            : KimiCredentials.tokenExpiredMessage
    }
}

/// Inline action shown below the Kimi tiles when the stored token can no
/// longer be used. Spawns `kimi login` and polls for the credentials file
/// to update — the chip recovers on its own when the fresh token lands.
struct ReauthButton: View {
    ...
    var body: some View {
        Button {
            store.reauthenticateKimi()
        } label: {
            Text(store.kimiReauthInProgress ? L10n.tr("waiting for browser…") : L10n.tr("Re-authenticate"))
                .font(Typography.label)
                .foregroundStyle(.white.opacity(hovered && !store.kimiReauthInProgress ? 0.95 : 0.72))
                .padding(.horizontal, 8)
                .padding(.vertical, 3)
                .background(
                    RoundedRectangle(cornerRadius: 5)
                        .fill(.white.opacity(hovered && !store.kimiReauthInProgress ? 0.08 : 0.04))
                )
                .contentShape(RoundedRectangle(cornerRadius: 5))
        }
        .buttonStyle(.plain)
        .disabled(store.kimiReauthInProgress)
        .onHover { hovered = $0 }
    }
}
