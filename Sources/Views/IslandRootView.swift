...
struct IslandRootView: View {
    ...
    /// re-decoded every logo on every render — inside a 120Hz TimelineView
    /// that's hundreds of main-thread decodes/sec. Cache once on appear,
    /// keyed by provider: the logo travels with the provider, so a slot
    /// swap only re-keys the lookup instead of hitting the disk.
    @State private var logos: [AlertEngine.Provider: NSImage] = [:]
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .background {
                ...
                .overlay(alignment: .topLeading) {
                    // Side is a property of the slot now, not the provider —
                    // any provider can sit left or right of the notch.
                    if let provider = slots.leftProvider {
                        LogoOverlay(
                ...
                .overlay(alignment: .topTrailing) {
                    if let provider = slots.rightProvider {
                        LogoOverlay(
                            image: logos[provider],
                            tint: provider.brandColor,
                            provider: provider,
                ...
                .overlay(alignment: .topLeading) {
                    ...
                    if model.state != .compact, let provider = slots.leftProvider {
                        PeekPillOverlay(
                            provider: provider,
                            edge: .leading,
                ...
                .overlay(alignment: .topTrailing) {
                    if model.state != .compact, let provider = slots.rightProvider {
                        PeekPillOverlay(
                            provider: provider,
                            edge: .trailing,
                            topPadding: max(0, (model.notch.height - 14) / 2),
        ...
        .onAppear {
            // Decode every provider's logo up front: the slot assignment
            // can change at runtime (Settings), so loading lazily per side
            // would stall on disk the first time a new provider takes a
            // slot. Cheap enough — five one-time decodes.
            for provider in AlertEngine.Provider.allCases where logos[provider] == nil {
                let resource = provider.logoResource
                logos[provider] = Bundle.main.url(forResource: resource.name, withExtension: resource.ext)
...
private struct LogoOverlay: View {
    ...
    /// Outer horizontal edge the logo hugs — `.leading` for the left slot,
    /// `.trailing` for the right. Side comes from the slot, never from the
    /// provider identity.
    let edge: Edge.Set
...
private struct PeekPillOverlay: View {
    let provider: AlertEngine.Provider
    /// Outer silhouette edge the pill is pinned to. Comes from the slot,
    /// so alignment / padding / slide-in offset all follow the side the
    /// provider currently sits on.
    let edge: Edge.Set
    let topPadding: CGFloat
    ...
    var body: some View {
        ...
        .animation(.openMorph, value: isVisible)
        .offset(x: pillsVisible ? 0 : (edge == .leading ? -6 : 6))
        .allowsHitTesting(false)
    ...
    private var currentWindow: WindowUsage {
        usageStore.usage[provider]?.fiveHour ?? .unknown
    }

    private var severity: AlertEngine.Severity {
        alerts.severities[provider] ?? .none
    }
