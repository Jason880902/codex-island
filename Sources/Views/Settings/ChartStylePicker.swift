    ...
    private func preview(for style: ChartStyle) -> some View {
        ...
        case .ring:
            ZStack {
                ...
                Circle()
                    .trim(from: 0, to: 0.35)
                    .stroke(kimi, style: StrokeStyle(lineWidth: 3, lineCap: .round))
            ...
            HStack(spacing: 1.5) {
                ForEach(0..<8) { i in
                    RoundedRectangle(cornerRadius: 0.75)
                        .fill(i < 3 ? kimi : .white.opacity(0.10))
        ...
        case .spark:
            SparkPath()
                .stroke(kimi, style: StrokeStyle(lineWidth: 1.5, lineCap: .round, lineJoin: .round))
