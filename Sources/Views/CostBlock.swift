...
struct CostBlock: View {
    ...
    /// can pick the right per-plan baseline (Codex Plus $20 / Pro $200,
    /// Claude Pro $20 / Max $100; the other providers have no published
    /// USD tier price) and the caption can name the plan.
...
struct CostTile: View {
    ...
    private func barColumn(
        ...
        return VStack(spacing: 3) {
            Text(displayAmount ?? formatBarDollars(amount))
    ...
    /// detected plan tier. Codex auto-maps from the API's "plan_type":
    /// Plus $20 / Pro $200. Claude maps from the plan tier the usage
    /// fetch read off the Keychain credentials (pro $20 / max $100).
    /// Kimi is a flat subscription whose usage API doesn't quote a USD
    /// price, and grok/glm are tracked subscription-side too, so all
    /// three return nil here — the multiplier hero renders their plan
    /// bar without a dollar figure.
    private var subscriptionUSD: Double? {
        switch provider {
        case .kimi, .grok, .glm:
            return nil
        case .codex:
            switch usageStore.usage[.codex]?.plan?.lowercased() {
            case "plus": return 20
            case "pro":  return 200
            default:     return nil
            }
        case .claude:
            guard let tier = claudeSubscriptionTier else { return nil }
            // Max has two USD tiers ($100 / $200); anchor the multiplier
            // to the conservative lower one so the bar never overstates
            // the plan's value (upstream makes the same choice).
            return tier == "pro" ? 20 : 100
        }
    }

    /// Claude's subscription tier, normalized to "pro" / "max" (nil when
    /// logged out or on an unrecognized tier). The raw value is the
    /// Keychain credentials' subscriptionType, which the usage fetch pipes
    /// onto `AppUsage.plan` (see `ClaudeCredentials.resolveUsage`) — read
    /// it from there rather than calling `ClaudeCredentials` directly,
    /// whose `readClaudeCreds()` is private to the usage layer.
    private var claudeSubscriptionTier: String? {
        guard let raw = usageStore.usage[.claude]?.plan?.lowercased()
        else { return nil }
    ...
    /// Display name of the active plan (Intermediate / Plus / Pro / Max).
    /// Used as the label under the plan bar so the user always knows what
    /// the comparison is anchored to. Kimi's membership level arrives
    /// display-ready from the usage fetch ("Intermediate"); Claude's tier
    /// is capitalized from the Keychain subscriptionType.
    private var planLabel: String? {
        switch provider {
        case .kimi:
            return usageStore.usage[.kimi]?.plan
        case .codex:
            switch usageStore.usage[.codex]?.plan?.lowercased() {
        ...
        case .claude:
            return claudeSubscriptionTier?.capitalized
        case .grok, .glm:
            return nil
        }
    }
    ...
    /// The plan bar's figure. Kimi has no published USD tier price
    /// (`subscriptionUSD == nil`), so its bar shows an em dash instead of
    /// a misleading "$0.00".
    private var planAmountText: String {
        if provider == .kimi && subscriptionUSD == nil { return "—" }
        return formatBarDollars(planAmount)
    }
    ...
    /// input + output only, excluding cache reads.
