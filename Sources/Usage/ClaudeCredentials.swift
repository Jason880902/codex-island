...
enum ClaudeCredentials {
    ...
    /// True when BOTH windows carry the server-side revocation sentinel: the
    /// keychain token is missing a scope the usage endpoint now requires and
    /// only a fresh `claude /login` recovers — a refresh re-issues the same
    /// scope set, so the session is dead until re-minted. `UsageStore`'s
    /// "don't clobber good values" retention makes an exception ONLY for
    /// this — a revoked token never comes back, so keeping stale numbers
    /// would mislead. Local expiry is deliberately excluded: Claude Code
    /// refreshes the keychain item on its own schedule (the access token
    /// lasts ~8h), so the retained value self-heals on a later poll.
    /// Blanking the peek pill to "—%" through every refresh gap was the
    /// visible bug on the Kimi side; same lesson applied here.
    ...
    ///
    /// Internal (not private) so SettingsView can show the login state and
    /// CostBlock can read the subscription tier directly.
    static func readClaudeCreds() -> ClaudeCreds? {
